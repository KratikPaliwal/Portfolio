            ...
            <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
                {experiences.map((exp, index) => (
                    ...
                    >
                        ...
                        {/* Content */}
                        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] glass-card p-8 rounded-[2.5rem] ml-16 md:ml-0">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
                                ...
                                <div className="flex flex-col items-start sm:items-end">
                                    <span className="flex items-center gap-2 text-xs font-bold text-purple-400/80 uppercase tracking-widest">
                            ...
                            <ul className="space-y-3 mb-6">
                                {exp.description.map((item, i) => (
                                    <li key={i} className="text-gray-400 text-sm leading-relaxed flex gap-3">
                                        <span className="mt-2 w-1.5 h-1.5 rounded-full bg-purple-500/40 shrink-0" />
                                        {item}
                                    </li>
                                ))}
                            </ul>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {exp.skills.map((skill, i) => (
                                    <span key={i} className="px-3 py-1 rounded-full bg-white/5 border border-white/5 text-[10px] font-bold text-gray-300 uppercase tracking-widest">
                                        {skill}
                                    </span>
                                ))}
                            </div>
